...
Step-by-step install commands, plus how to run the frontend, backend and Form Builder locally, are in [`docs/01-developer-environment-setup.md`](docs/01-developer-environment-setup.md).
...
> **Status note (2026-09-21):** the automatic ◀ Back / Next ▶ row can now be turned off per screen. The app always added that row under every screen, so a screen that wanted only its own buttons (e.g. a lone Submit on the last screen) still got a Next. `ScreenConfig.showNavigation: false` hides it; leaving it out means shown, so existing forms are unchanged. The Review screen always keeps its own Back/Submit. Implemented in `frontend/src/screens/FormApp.tsx`, typed in `frontend/src/types/config.ts` and mirrored in `backend/app/models/schemas.py`. With the row off, the screen's own `button` actions (Section 4.2) are the only way to move on or back, so the Form Builder warns when a screen after the first has it off and no Back/Skip button (see the Section 7 status note of 2026-09-24).
...
- `textblock` (Text block) — plain display text, such as a welcome message: bilingual `FieldConfig.content`, an optional `align` (left by default, or center/right), and the per-field `textColor`/`fontSize`/`background`/`width` controls, which style the text itself here rather than a label. It is **not an input** — no label, value, required flag or help text — so it is left off the Review screen, and it can be nested in an accordion. Line breaks the author types are kept. **Implemented** (2026-09-21): `TextBlockField.tsx`, rendered by `FieldRenderer`; the `textblock` type is in `frontend/src/types/config.ts`, with the shared `content`/`align` comments mirrored in `backend/app/models/schemas.py` (its `type` is a free string, so no schema change was needed). It exists because the `text` type is an input box for end users and could not carry a message.
...
- `accordion` (Collapsible section) — an expandable header with optional intro text and nested fields, one level deep (accordion-in-accordion not supported). **Implemented**: `AccordionField.tsx` recursively renders `FieldConfig.children` (including nested `button`/`image`/`textblock` fields) via `FieldRenderer` itself. `FieldConfig.children?: FieldConfig[]` is the nested shape that was an open question below — resolved by just extending the existing flat type with an optional self-referencing array, no separate schema needed.
...
> **Status note (2026-09-24):** one deliberate exception to the palette: the `rating` field's filled stars are **gold** (`#E8A317`) instead of Morado, because gold reads as a star rating far better. It is added to the Tailwind theme as `gold` in `frontend/tailwind.config.ts` (with a comment marking it as outside this palette), used by `frontend/src/components/fields/RatingField.tsx`, and mirrored as `--gold` in the Form Builder mockup. Empty stars stay Beige. It is not offered as a swatch, so builders still can't pick it for anything else.
...
> **Status note (2026-09-21):** the "Partial/autosave of in-progress forms" exclusion above (and Section 4.5's "No partial/autosave") is about **end users filling in a published form** — that stays out of scope. It does not cover the **Form Builder** (`design/mockups/form-builder.html`), which now has an authoring model of its own: it opens on a **blank template that stays blank** (view-only — never edited or saved); **New app** makes a named copy the author owns, and that copy **autosaves** as they work (there is also a Save draft button, and a Drafts panel to open, delete, or switch back to the blank template). The last-open app is restored on load. A **Preview** toggle turns the canvas into a live, try-it-out form — typing, dropdowns, checkboxes, radios, toggles, ratings and the form's own buttons work, nothing is saved or sent; photo, signature, QR and SQL-dropdown fields stay placeholders, and there is no required-field validation or automatic Back/Next row in it yet. Drafts are `localStorage`-only for now — the mockup has no backend, so they don't follow an author across browsers, machines, or addresses (`file://` and `localhost` are separate stores). For local use the builder is served by the frontend dev server at `http://localhost:5173/builder.html` (a dev-only plugin in `frontend/vite.config.ts`, not part of production builds); to test a built form in the real app, export its JSON to `frontend/src/forms/<id>/config.json`, where the folder name must equal the JSON's `id`. **Not yet decided:** how authored forms get from draft to live — i.e., a Publish / promote-to-production step that stores the config server-side (today forms are `frontend/src/forms/<id>/config.json` files baked in at build time). That decision ties into the dev/production environments in Section 8 and the "smaller technical subset owns the backend" model in Section 6.

> **Status note (2026-09-24):** updates to the Form Builder note above; where they differ, this one is current.
> 1. **Save to app replaces the manual export.** A form no longer has to be exported and copied into `frontend/src/forms/<id>/config.json` by hand: the builder's **Save to app** button writes that file itself, through a dev-only endpoint (`/__builder/save-form`, the `saveFormToApp` plugin in `frontend/vite.config.ts`). It only accepts requests from the same machine and origin, and is left out of `vite build`, so it exists only while running `npm run dev`. The form id comes from the app name: letters, numbers, `-` and `_`, and it can't start with `_` (folders starting with `_` are the repo's own test forms). Replacing an existing form asks first and keeps the old version as `config.previous.json`, which is git-ignored. This is a local development shortcut, not the Publish step, which is still undecided. Saved forms still ship by being committed with the code. The first two forms made this way are `frontend/src/forms/test_web_app_template/` and `frontend/src/forms/test_web_app_1/`.
> 2. **Draft locking.** Each draft has **Lock / Unlock**. A locked draft can't be deleted from the Drafts panel, and autosave keeps the lock.
> 3. **Automatic Back / Next in the builder.** Screen properties have a **Show automatic Back / Next buttons** checkbox, which sets `showNavigation` (Section 4.1). The canvas now draws that row: faint while editing, working in Preview. So the note above is out of date where it says Preview has no automatic Back/Next row. The builder warns when a screen after the first has the row off and no Back or Skip button of its own. Preview still has no required-field validation.
